...
interface Ticket {
  ...
  status: string;
  updatedAt: string;
  customer: { name: string; phone: string; organization?: { name: string; displayName: string | null } };
...
export function Tickets() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["tickets", page, pageSize, statusFilter, sortKey, sortDir],
    queryFn: async () =>
      (
        await api.get("/tickets", {
          params: { page, pageSize, status: statusFilter || undefined, sortBy: sortKey ?? undefined, sortDir },
        })
      ).data as { items: Ticket[]; total: number; page: number; pageSize: number },
  });
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!isLoading && (data?.items.length ?? 0) === 0 && (
                <tr>
                  <td className="p-3 text-muted-foreground" colSpan={isSuperAdmin ? 9 : 8}>
                    No tickets found.
                  </td>
                </tr>
              )}
                ...
                >
                  {isSuperAdmin && (
                    <td className="p-3 text-muted-foreground">
                      {t.customer.organization?.displayName || t.customer.organization?.name || "-"}
                    </td>
                  )}
                  <td className="p-3 font-mono text-xs">{t.ticketNumber}</td>
                  <td className="p-3">
                    {t.customer.name}
                    <span className="ml-1 text-xs text-muted-foreground">({t.customer.phone})</span>
                  </td>
