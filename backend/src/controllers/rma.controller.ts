...
export async function createRma(req: Request, res: Response) {
  ...
  const imei = await prisma.imeiRecord.findUniqueOrThrow({ where: { id: data.imeiRecordId } });
  if (imei.status === "RMA") {
...
export async function deleteRmaCore(id: string, userId?: string | null) {
  const rma = await prisma.rma.findUniqueOrThrow({ where: { id }, include: { imeiRecord: true } });
...
export async function deleteRma(req: Request, res: Response) {
  await deleteRmaCore(req.params.id, req.user!.sub);
  res.status(204).send();
}
